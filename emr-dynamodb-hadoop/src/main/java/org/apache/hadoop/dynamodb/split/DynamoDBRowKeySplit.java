...
public class DynamoDBRowKeySplit extends DynamoDBSegmentsSplit {
  ...
  private DynamoDBQueryFilter buildFilterPushdown() {
    ...
    // We add filter info that's common to all splits here, like index name, sort criteria, etc.
    // In the MultiKeyQueryRecordReadRequest we then add split-specific criteria, like rowKey
    ...
    // BETWEEN is inclusive on both ends so take a second off the end to make in exclusive
    filterPushdown.addKeyCondition(
        new DynamoDBNAryFilter(sortKeyName, DynamoDBFilterOperator.BETWEEN,
            ...
            Long.toString(sortKeyMaxExclusive - 1)));
